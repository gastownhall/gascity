...
[Troubleshooting](../getting-started/troubleshooting.md). For Dolt store
recovery, see [Dolt bloat recovery](dolt-bloat-recovery.md).
</Note>
...
and consult the [installation troubleshooting](../getting-started/troubleshooting.md)
...
**Background**: [PRD `ga-sn06`](https://github.com/gastownhall/gascity/issues/sn06).
...
**Background**: [PRD `ga-7kwr`](https://github.com/gastownhall/gascity/issues/7kwr).
...
<CodeGroup>
```bash Migrate your pack to v2 (recommended)
# See the migration guide:
#   https://docs.gascityhall.com/guides/migrating-to-pack-vnext
# Move each [[agent]] block to its own agents/<name>/ directory.
```

```toml Override the system pack with fallback = true
# In your pack's agents/<name>/agent.toml (v2 layout):
[agent]
name = "mayor"
fallback = true   # use only if no other declaration exists
# … the rest of your agent config …
```
</CodeGroup>

The recommended path is migration. The `fallback = true` escape hatch
is for packs that intentionally wrap a system pack and need the local
declaration to win when both are present.

**Background**: [Pack v1 → v2 migration guide](../guides/migrating-to-pack-vnext.md) ·
...
For a full mapping of removed/renamed fields, see the
[Pack v1 → v2 migration guide](../guides/migrating-to-pack-vnext.md).
