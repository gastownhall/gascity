...
| `min_wake_interval` | string |  |  | MinWakeInterval, if non-empty, prevents the controller from waking this agent more often than this duration since its last wake. Duration string (e.g., "5m"). Empty or zero disables the throttle. Useful for fresh-mode utility agents (watchdogs, ticks) whose drain-followup pokes would otherwise produce a tight respawn loop. Throttle is measured from the bead's creation_complete_at timestamp (which survives the death/churn gap that clears last_woke_at); deferred starts log the "deferred_by_min_wake_interval" outcome and retry on the next reconciler tick. |
