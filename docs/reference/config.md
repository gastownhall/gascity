...
| `cancel_timeout` | string |  | `10s` | CancelTimeout is how long interrupting an ACP session waits for the agent to end its turn after session/cancel before falling back to SIGINT. The interrupt never kills the agent. Duration string (e.g., "10s", "30s"). Defaults to "10s"; non-positive or unparseable values fall back to the default. |
