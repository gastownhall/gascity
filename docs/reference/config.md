...
| `postgres_url` | string |  |  | PostgresURL is the password-free connection URL used by the native upstream bd Postgres backend. Supply the password through the Beads/GC credential environment rather than embedding it in city.toml. |
| `postgres_schema` | string |  |  | PostgresSchema optionally pins the city scope schema. Rig schemas are derived from the backend pack's scope namespace policy. |
| `sqlite_path` | string |  |  | SQLitePath overrides the native SQLite database path. Relative paths are resolved beneath each scope's .beads directory. |
