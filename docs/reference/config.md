...
| `stop_grace` | string |  | `5s` | StopGrace is how long stopping an ACP session waits after SIGTERM before escalating to SIGKILL. Raise it for agents that need longer to drain in-flight tool calls on shutdown. Duration string (e.g., "5s", "30s"). Defaults to "5s"; non-positive or unparseable values fall back to the default. |
