...
| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `mode` | string |  |  | Mode selects the Dolt transport for managed-local Beads scopes. The empty value preserves the direct SQL-server default; set to "proxied-server" to opt into Beads' proxied UOW path. Host/port may be supplied for a proxied adapter targeting an external Dolt server. Enum: `server` |
