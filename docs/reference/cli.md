...
| [gc gastown](#gc-gastown) | Commands from the gastown import |
...
| [gc workflows](#gc-workflows) | Commands from the workflows import |
...
| [gc discord retry-peer-fanout](#gc-discord-retry-peer-fanout) | Retry failed peer fanout targets for a saved room publish without reposting to Discord |
...
Commands from the gastown import
...
| Subcommand | Description |
|------------|-------------|
| [gc gastown status](#gc-gastown-status) |  |
...
Commands from the workflows import
...
| Subcommand | Description |
|------------|-------------|
| [gc workflows pr-review](#gc-workflows-pr-review) | Create and retry PR-review workflow routing beads |

## gc workflows pr-review

Create and retry PR-review workflow source beads.

Usage:

```bash
gc workflows pr-review create https://github.com/owner/repo/pull/123
gc workflows pr-review create https://github.com/owner/repo/pull/123 --allow-closed
gc workflows pr-review retry &lt;bead-id&gt;
gc workflows pr-review config set-repo owner/repo --city /abs/city --rig &lt;rig&gt; --base-branch main
gc workflows pr-review config remove-repo owner/repo
gc workflows pr-review config set-skip-gemini true
gc workflows pr-review config show
```

Run this command from inside the target city so Gas City can discover imported
pack commands. If discovery is unavailable, run the pack script directly:

```bash
python3 /data/projects/gas-city-inc/packs/workflows/scripts/pr_review.py &lt;subcommand&gt;
```
