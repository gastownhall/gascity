...
This proposal does not cover `.gc/` internals beyond what the pack changes require, package registry/distribution (though it's compatible with a future registry), or immediate breakage of existing cities (migration path required).
## Proposed change
...
- **Migration:** Hard cutover. `gc doctor` detects V1 patterns and produces actionable messages. `gc import migrate` handles the mechanical conversion. After one release of deprecation warnings, the V2 loader will refuse V1 shapes.
...
- **`[commands]` structure:** How should a pack expose multiple top-level CLI verbs? This would allow a single pack to provide both `gc import` and `gc registry` commands. Design in progress; deferred from v1.
