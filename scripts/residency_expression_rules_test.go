...
type residencyGuardedNames struct {
	bare         map[string]bool // BeadStores, cliSoleClassBindingStore, ...
	selector     map[string]bool // storeFor, from the receiver-blind row `\.storeFor\(`
	qualified    map[string]bool // "storeref.EachLeg"
...
func TestResidencyGuardedNameDerivation(t *testing.T) {
	...
	for _, want := range []string{"BeadStores", "cliSoleClassBinding", "cliSoleClassBindingStore"} {
		if !names.bare[want] {
			t.Errorf("%q is grep vocabulary but not alias vocabulary", want)
		}
	}
	if !names.selector["storeFor"] {
		t.Error(`the receiver-blind row \.storeFor\( did not reduce to the selector "storeFor"`)
	}
	for _, want := range []string{"storeref.EachLeg", "storeref.ClassCandidates"} {
		if !names.qualified[want] {
			t.Errorf("%q is grep vocabulary but not alias vocabulary", want)
		}
	}
	// The selector half of every qualified row is guarded on its own, because the
	// grep row names one package qualifier and an import can be renamed.
	for _, want := range []string{"EachLeg", "ClassCandidates"} {
...
func TestResidencyASTExpressionRatchet(t *testing.T) {
	...
	baseline, err := readResidencyBaseline(residencyBaselinePath(t), func(p string) bool {
		return p == residencyAliasPattern || p == residencyLegChainPattern
	})
	if err != nil {
