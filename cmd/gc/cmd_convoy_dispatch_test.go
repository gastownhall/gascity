...
func TestWorkflowServeControlReadyQueryUsesControlTiers(t *testing.T) {
	...
	for _, want := range []string{
		...
		`bd --readonly --sandbox ready --metadata-field "gc.run_target=$route" --unassigned --exclude-type=epic --exclude-label "hold:mayor" --exclude-label "hold:external" --json --sort priority --limit=20`,
		`bd --readonly --sandbox ready --metadata-field "gc.routed_to=$route" --unassigned --exclude-type=epic --exclude-label "hold:mayor" --exclude-label "hold:external" --json --sort priority --limit=20`,
...
func TestWorkflowServeControlReadyQueryBD105IncludesEphemeral(t *testing.T) {
	...
	for _, want := range []string{
		...
		`bd --readonly --sandbox ready --include-ephemeral --metadata-field "gc.run_target=$route" --unassigned --exclude-type=epic --exclude-label "hold:mayor" --exclude-label "hold:external" --json --sort priority --limit=20`,
		`bd --readonly --sandbox ready --include-ephemeral --metadata-field "gc.routed_to=$route" --unassigned --exclude-type=epic --exclude-label "hold:mayor" --exclude-label "hold:external" --json --sort priority --limit=20`,
	} {
		if !strings.Contains(query, want) {
...
case "$*" in
  ...
  "--readonly --sandbox ready --assignee=gascity--control-dispatcher --exclude-type=epic --json --limit=20")
    printf '[{"id":"ga-ready"}]'
    ;;
  "--readonly --sandbox ready --metadata-field gc.run_target=gascity/control-dispatcher --unassigned --exclude-type=epic --exclude-label hold:mayor --exclude-label hold:external --json --sort priority --limit=20")
    printf '[{"id":"ga-routed"}]'
    ;;
  *)
    printf '[]'
...
set -eu
case "$*" in
  "--readonly --sandbox ready --metadata-field gc.routed_to=gascity/control-dispatcher --unassigned --exclude-type=epic --exclude-label hold:mayor --exclude-label hold:external --json --sort priority --limit=20")
...
`)
	...
	wantArgs := []string{"--readonly", "--sandbox", "ready", "--metadata-field", "gc.run_target=my rig/control-dispatcher", "--unassigned", "--exclude-type=epic", "--exclude-label", "hold:mayor", "--exclude-label", "hold:external", "--json", "--sort", "priority", "--limit=20"}
	if !slices.Equal(gotArgs, wantArgs) {
...
set -eu
case "$*" in
  "--readonly --sandbox ready --metadata-field gc.run_target=gascity/workflow-control --unassigned --exclude-type=epic --exclude-label hold:mayor --exclude-label hold:external --json --sort priority --limit=20")
