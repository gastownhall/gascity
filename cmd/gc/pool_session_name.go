...
func releaseOrphanedPoolAssignments(
	...
	for i, wb := range assignedWorkBeads {
		...
		if assignee == "" {
			...
			// handoff) AND the gc.routed_at settle window has elapsed, the owning
			// session is the stale source, not the legitimate owner —
			// assigneeRoutedAwayFromOwnAgent detects that (store-ref-aware,
			// settle-gated, over all owners, biased to preserve) and lets the
