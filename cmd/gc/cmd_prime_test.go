...
`), 0o644); err != nil {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.liveSession {
				sessionID := createPrimeHookSession(t, cityDir, "gastown--worker", "worker", "active")
...
`, tt.agentName, tt.agentDir, tt.promptFile)
			...
			sessionID := createPrimeHookSession(t, cityDir, sessionName, tt.beaconAgent, "active")
			t.Setenv("GC_SESSION_ID", sessionID)
...
func createPrimeHookSession(t *testing.T, cityDir, sessionName, template, state string) string {
	t.Helper()
