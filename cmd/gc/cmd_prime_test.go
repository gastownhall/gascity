...
`), 0o644); err != nil {
	...
	seededSessionID, err := sessionpkg.NewStore(beads.SessionStore{Store: store}).CreateSession(sessionpkg.CreateSpec{
		Title:     "worker",
		AgentName: "worker",
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.sessionID != "" {
				// A SessionStart hook with no live managed session writes
				// nothing at all (primeHookHasLiveManagedSession).
				if out != "" {
					t.Fatalf("stdout = %q, want empty for a dead session", out)
				}
				return
			}
