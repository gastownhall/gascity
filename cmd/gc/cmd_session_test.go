...
func TestSessionNewExposesWaitTimeoutFlag(t *testing.T) {
	var stdout, stderr bytes.Buffer
	cmd := newSessionNewCmd(&stdout, &stderr)

	flag := cmd.Flags().Lookup("wait-timeout")
	if flag == nil {
		t.Fatal("gc session new missing --wait-timeout flag")
	}
	if flag.DefValue != "30s" {
		t.Fatalf("--wait-timeout default = %q, want 30s", flag.DefValue)
	}
	if !strings.Contains(flag.Usage, "session to start") {
		t.Fatalf("--wait-timeout usage = %q, want session start wording", flag.Usage)
	}
}

func TestWaitForSessionZeroTimeoutStillObservesRunningSession(t *testing.T) {
	sp := runtime.NewFake()
	...
	if err := waitForSession(sp, "worker", 0, nil, "gc-1", &stderr); err != nil {
		t.Fatalf("waitForSession with zero timeout = %v, want nil for running session", err)
	}
}
