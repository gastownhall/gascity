...
func shutdownBeadsProvider(cityPath string) error {
	...
	if strings.HasPrefix(provider, "exec:") {
		if cityUsesDoltliteBeadsBackend(cityPath) {
			script := strings.TrimPrefix(provider, "exec:")
			return runProviderOpWithEnv(script, providerLifecycleProcessEnv(cityPath, provider), "stop")
		}
		if providerUsesBdStoreContract(provider) && isExternalDolt(cityPath) {
			return clearManagedDoltRuntimeStateIfOwned(cityPath)
		}
...
func healthBeadsProvider(cityPath string) error {
	...
	if cityUsesDoltliteBeadsBackend(cityPath) {
		provider := beadsProvider(cityPath)
		if strings.HasPrefix(provider, "exec:") {
			return runProviderOpWithEnv(strings.TrimPrefix(provider, "exec:"), providerLifecycleProcessEnv(cityPath, provider), "health")
		}
		return nil
	}
	provider := beadsProvider(cityPath)
...
func providerLifecycleProcessEnv(cityPath, provider string) []string {
	...
	env = append(env, "GC_BEADS_BACKEND="+beadsBackend(cityPath))
	if cityUsesDoltliteBeadsBackend(cityPath) {
		for _, key := range append(projectedDoltEnvKeys, "BEADS_DOLT_AUTO_START") {
...
// timeout because dolt/doltlite store startup and first-time schema checks can
// take 30+ seconds for large data dirs. All other operations use 30s.
