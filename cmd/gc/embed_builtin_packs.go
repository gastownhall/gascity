...
// Dolt is included only for the Dolt-backed bd mode; doltlite uses bd without
// a managed Dolt server. Gastown is never auto-included — it requires an
// explicit workspace.includes entry.
func builtinPackIncludes(cityPath string) []string {
	...
	// normalized back to "bd", so it only needs the bd pack. Dolt is included
	// separately unless the bd backend is doltlite.
	...
	usesBd := providerUsesBdStoreContract(normalizedProvider)
	usesDoltlite := usesBd && beadsBackend(cityPath) == "doltlite"
	if usesBd {
		if bdPath := filepath.Join(systemRoot, "bd"); packExists(bdPath) {
			includes = append(includes, bdPath)
		}
	}
	if (usesBd && !usesDoltlite) || usesDirectExecLifecycle {
...
func peekBeadsConfigFromFile(tomlPath string) peekBeadsConfig {
	data, err := os.ReadFile(tomlPath)
	if err != nil {
		return peekBeadsConfig{}
	}
	var peek peekBeadsConfig
...
func peekBeadsProvider(tomlPath string) string {
	peek := peekBeadsConfigFromFile(tomlPath)
