...
func TestCheckHardDependenciesDoesNotRequireDoltForDoltliteBdBackend(t *testing.T) {
	cityDir := t.TempDir()
...
`), 0o644); err != nil {
	...
	initRunVersion = func(binary string) (string, error) {
		if binary == "dolt" {
			t.Fatal("dolt version should not be checked for doltlite backend")
		}
		switch binary {
		case "bd":
			return "bd version " + bdMinVersion, nil
	...
	if missing := checkHardDependencies(cityDir); len(missing) != 0 {
		t.Fatalf("missing deps = %#v, want no missing deps without dolt for doltlite backend", missing)
	}
}
