...
func TestPhase0CLISessionTargetingSurfaces_RejectTemplateFactoryTargets(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "gc mail send",
			run: func(stdout, stderr *bytes.Buffer) int {
				return cmdMailSendJSON([]string{"template:worker", "hello"}, false, false, "", "", "", "", mailSendAnnotations{}, false, stdout, stderr)
			},
		},
		{
...
func TestPhase0CLISessionTargetingSurfaces_BareConfigNameDoesNotMaterializeOrdinarySession(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "gc mail send",
			run: func(stdout, stderr *bytes.Buffer) int {
				return cmdMailSendJSON([]string{"worker", "hello"}, false, false, "", "", "", "", mailSendAnnotations{}, false, stdout, stderr)
			},
		},
		{
			name: "gc session nudge",
