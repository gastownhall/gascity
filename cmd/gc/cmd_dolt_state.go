...
func newDoltStateCmd(stdout, stderr io.Writer) *cobra.Command {
	...
	resetProbe := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			if !forceReset {
				fmt.Fprintf(stderr, cmdName("dolt-state reset-probe")+": refusing to reset health probe artifacts without --force; %s may contain a legacy bead store in old metadata\n", managedDoltProbeDatabase) //nolint:errcheck
				return errExit
			}
	...
	waitReady := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if err != nil {
				fmt.Fprintf(stderr, cmdName("dolt-state wait-ready")+": invalid --pid %q: %v\n", pidText, err) //nolint:errcheck
				return errExit
			}
