...
func allSessionBeadsWithClosed(t *testing.T, store beads.Store) []beads.Bead {
	t.Helper()
...
func TestSyncSessionBeads_Idempotent(t *testing.T) {
	...
	all := allSessionBeadsWithClosed(t, store)
...
func TestSyncSessionBeads_PreservesCreatingForPendingCreateClaim(t *testing.T) {
	store := beads.NewMemStore()
	clk := &clock.Fake{Time: time.Date(2026, 3, 7, 12, 0, 0, 0, time.UTC)}
	sp := runtime.NewFake()
	...
	ds := map[string]TemplateParams{
		"worker": {TemplateName: "worker", Command: "codex"},
	}
