...
type directWorkQueryTestStore struct {
	*beads.MemStore
	targets    []string
	identities []string
}

func (s *directWorkQueryTestStore) DefaultWorkQueryHasReadyWork(targets []string, identities []string, includeRouted bool) (bool, error) {
	s.targets = append([]string(nil), targets...)
	s.identities = append([]string(nil), identities...)
	return includeRouted && sessionReconcileSlicesContains(targets, "worker"), nil
}

func sessionReconcileSlicesContains(values []string, want string) bool {
...
func TestComputeWorkSet_DefaultWorkQueryUsesDirectStore(t *testing.T) {
	cfg := &config.City{
		Workspace: config.Workspace{Name: "test-city"},
		Agents:    []config.Agent{{Name: "worker"}},
	}
	store := &directWorkQueryTestStore{MemStore: beads.NewMemStore()}
	runnerCalled := false
	...
	work := computeWorkSet(cfg, runner, "test-city", "/tmp", store, nil, nil)
	if !work["worker"] {
		t.Fatal("expected worker to have work from direct store")
	}
	if runnerCalled {
		t.Fatal("default work query should not shell out when direct store handles it")
	}
	if len(store.targets) != 1 || store.targets[0] != "worker" {
		t.Fatalf("targets = %v, want [worker]", store.targets)
	}
}

func TestComputeWorkSet_DefaultWorkQueryUsesDirectStoreThroughCache(t *testing.T) {
	cfg := &config.City{
		Workspace: config.Workspace{Name: "test-city"},
		Agents:    []config.Agent{{Name: "worker"}},
	}
	backing := &directWorkQueryTestStore{MemStore: beads.NewMemStore()}
	...
	work := computeWorkSet(cfg, runner, "test-city", "/tmp", store, nil, nil)
	if !work["worker"] {
		t.Fatal("expected worker to have work from cached direct store")
	}
	if runnerCalled {
		t.Fatal("default work query should not shell out when cached direct store handles it")
	}
	if len(backing.targets) != 1 || backing.targets[0] != "worker" {
		t.Fatalf("targets = %v, want [worker]", backing.targets)
	}
}
