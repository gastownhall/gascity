...
func (c SpawnStormConfig) effective() SpawnStormConfig {
	...
	if c.InitialBackoff <= 0 {
		// Threshold-of-5-in-5min already implies sustained badness, so a
		// short initial backoff suffices to break a tight spawn loop while
		// keeping the cost-of-being-wrong low (operators or natural
		// recovery aren't penalized for ten minutes on a single false
		// positive). The exponential doubling on continued drain-without-
		// claim outcomes still ramps quickly under sustained pressure.
		c.InitialBackoff = 1 * time.Minute
	}
	if c.MaxBackoff <= 0 {
...
type templateStormState struct {
	...
	drains []time.Time
	// drainSessions is the parallel slice of session names that
	// produced each drain in `drains`. Same length, same order,
	// pruned in lockstep. Used to surface the full contributor list
	// in operator-facing diagnostics (mail body) when an episode
	// transition fires.
	drainSessions []string
...
func (s *SpawnStormSafetyNet) RecordDrainOutcome(template, sessionName string, claimed bool, now time.Time) bool {
	...
	// Slide the window forward before counting. drainSessions is the
	// parallel slice; prune it to the same head-index so the contributor
	// list stays aligned with the timestamps.
	st.drains, st.drainSessions = pruneDrainsBefore(st.drains, st.drainSessions, now.Add(-s.cfg.Window))
	st.drains = append(st.drains, now)
	st.drainSessions = append(st.drainSessions, sessionName)
...
// CurrentDrainContributors returns the session names that produced the
// drain-without-claim outcomes currently held in the sliding window for
// template, in arrival order. Returns nil when no template state exists.
// Caller receives a copy; mutating it does not affect detector state.
func (s *SpawnStormSafetyNet) CurrentDrainContributors(template string) []string {
	if s == nil {
		return nil
	}
	template = trimTemplate(template)
	...
	defer s.mu.Unlock()
	st := s.perTemplate[template]
	if st == nil || len(st.drainSessions) == 0 {
		return nil
	}
	out := make([]string, len(st.drainSessions))
	copy(out, st.drainSessions)
	return out
}

// pruneDrainsBefore drops elements from the head of ts (and the parallel
// names slice) whose timestamp is older than cutoff. The two slices must
// be the same length on entry; the result preserves that invariant.
func pruneDrainsBefore(ts []time.Time, names []string, cutoff time.Time) ([]time.Time, []string) {
	idx := 0
	...
	ts = append(ts[:0], ts[idx:]...)
	if idx <= len(names) {
		names = append(names[:0], names[idx:]...)
	}
	return ts, names
}

func doublingCappedAt(current, ceiling time.Duration) time.Duration {
	doubled := current * 2
	if doubled <= 0 || doubled > ceiling {
		return ceiling
	}
	return doubled
}
