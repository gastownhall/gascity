...
func TestPrefixedWorkQueryForProbe_UsesNamedSessionRuntimeName(t *testing.T) {
	...
	if !strings.Contains(command, `gc ready --metadata-field "gc.routed_to=$target"`) || !strings.Contains(command, "-- demo/witness") {
...
func TestPrefixedWorkQueryForProbeUsesBD105WorkQuery(t *testing.T) {
	...
	if !strings.Contains(command, "gc ready --include-ephemeral") {
