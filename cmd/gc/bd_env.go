...
func bdTransportRetryableError(cityPath, scopeRoot string, env map[string]string, err error) bool {
	if err == nil || cityUsesDoltliteBeadsBackend(cityPath) || !providerUsesBdStoreContract(rawBeadsProviderForScope(scopeRoot, cityPath)) || !managedBDRecoveryAllowed(cityPath, scopeRoot, env) {
		return false
	}
...
func bdRuntimeEnv(cityPath string) map[string]string {
	...
	env["GC_BEADS_BACKEND"] = beadsBackend(cityPath)
	if cityUsesDoltliteBeadsBackend(cityPath) {
		clearProjectedDoltEnv(env)
		mirrorBeadsDoltEnv(env)
		return env
...
func cityRuntimeProcessEnv(cityPath string) []string {
	...
	overrides := cityRuntimeEnvMapForCity(cityPath)
	overrides["GC_BEADS_BACKEND"] = beadsBackend(cityPath)
	if cityUsesDoltliteBeadsBackend(cityPath) {
		for _, key := range projectedDoltEnvKeys {
			overrides[key] = ""
		}
		overrides["BEADS_DOLT_AUTO_START"] = ""
