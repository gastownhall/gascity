...
func reconcileDetachedAtInfo(
	...
	// Preserve the historical best-effort treatment of ordinary handle/store
	// errors: only the T3 uncertainty sentinel fences lifecycle mutation.
...
// sessionIdleReferenceInfo is the best-effort report projection. Lifecycle
// callers use sessionIdleReferenceInfoWithError so an unavailable activity
// observation cannot be mistaken for an absent timestamp.
...
// configWakeSuppressedInfoWithError is the lifecycle form of
// configWakeSuppressedInfo. It preserves activity-observation uncertainty so
// the reconciler can defer the target instead of deriving an idle transition
