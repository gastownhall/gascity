...
committed workspace — e.g. from a bootstrap.sh shipped in the repo).`,
  ...
  gc init --template gascity --default-provider claude \
	...
	cmd.Flags().StringVar(&beadsTransportFlag, "beads-transport", "", "beads transport selector: direct or proxied (or "+envBeadsTransport+"); give with --beads-target. Default on bd >= 1.3.0 is proxied: bd owns the Dolt process, any bd read restarts it, and gc stop stops it. Use direct for the legacy gc-managed Dolt server")
	cmd.Flags().StringVar(&beadsTargetFlag, "beads-target", "", "beads target selector: local or external (or "+envBeadsTarget+"); give with --beads-transport. Default local. external requires --dolt-host/--dolt-port (or "+envDoltHost+"/"+envDoltPort+")")
