...
func buildDoctorChecks(cityPath string, cfg *config.City, cfgErr error, opts buildDoctorChecksOpts) []doctor.Check {
	...
	for _, check := range coreBinaryChecks(sessionProvider, exec.LookPath) {
		register(check)
	}
...
func registerCoreBinaryChecks(d *doctor.Doctor, sessionProvider string, lookPath doctor.LookPathFunc) {
	for _, check := range coreBinaryChecks(sessionProvider, lookPath) {
		d.Register(check)
	}
}

func coreBinaryChecks(sessionProvider string, lookPath doctor.LookPathFunc) []doctor.Check {
	if lookPath == nil {
	...
	for _, dep := range coreBinaryDependencies(sessionProvider, "", coreBinaryDependencyOptions{}) {
		checks = append(checks, newBinaryDependencyCheck(dep, lookPath))
	}
	return checks
}
