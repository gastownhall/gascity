...
// sessionStartOperation is the operation every session-start fixture in this
// file emits. Inlined as a constant to avoid an unparam lint on the helper: no
// test here needs a different operation today, and the "start_resolved"
// spelling is already covered where the parsing lives (internal/sessionoutcomes).
const sessionStartOperation = "start"

func sessionStartEvent(t *testing.T, seq uint64, ts time.Time, result, template, provider string, durationMs int64) events.Event {
	t.Helper()
...
func TestRunAnalyzeSessions_TableOutput(t *testing.T) {
	...
	now := time.Now().UTC()
	es := []events.Event{
		sessionStartEvent(t, 1, now, "succeeded", "polecat", "anthropic", 100),
		sessionStartEvent(t, 2, now, "failed", "polecat", "anthropic", 200),
	}
...
func TestRunAnalyzeSessions_TemplateFilter(t *testing.T) {
	...
	es := []events.Event{
		sessionStartEvent(t, 1, now, "succeeded", "polecat", "anthropic", 100),
		sessionStartEvent(t, 2, now, "succeeded", "manager", "openai", 100),
	}
	writeEventsFile(t, dir, es)
