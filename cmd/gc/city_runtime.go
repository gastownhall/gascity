...
func (cr *CityRuntime) tick(
	...
	// Detect pool instance deaths since last tick (skipped on stretched
	// patrol ticks: death events poke the reconciler directly).
	if runSessionPhases {
	...
	// A config change reconciles fully even inside the stretched window.
	...
	// Demand/desired-state/reconcile core. Skipped on patrol ticks inside
	// the stretched session-patrol window (see sessionPhasesDue) — event
	// pokes re-run it on demand; the stretched patrol scan is the safety net.
	if runSessionPhases {
		phaseStart = time.Now()
