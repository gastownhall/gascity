...
| Source debt ratchet | build_targets | test_target: 43 calls / 4 files | ga-4ag4p2.1 | declared go-test invocation sites in tracked build-orchestration files (Makefile, CI workflow YAML) cannot grow without an explicit, reviewed baseline update; each new test target's author declares it via a baseline bump with real ownership fields — same contract as every other resource dimension (ga-hpcma0) | n/a — preventive dimension, not paying down inherited debt | 2026-10-01 |
