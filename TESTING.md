...
| Audit baseline | all tracked test source | subprocess: 551 calls / 168 files (historical regex census: 495 / 135) | ga-80po0c.2 | tracked test source totals remain visible as audit evidence; ga-80po0c.2 owns this point-in-time source census | P0.4a | 2026-10-01 |
...
| Medium owner | `internal/hooks` package `hooks` | TestReconcileCodexHooksWaitsForCrossProcessHookSave: subprocess | ga-80po0c.2.1 | Codex hook cross-process path-lock regression is a checked Medium owner; the test-binary helper subprocess is confined to TestReconcileCodexHooksWaitsForCrossProcessHookSave, signals readiness over stdout, and is released over stdin | P0.4b | 2026-10-01 |
| Medium owner | `internal/runtime/exec` package `exec` | TestScreenSessionScriptOwnedPathContract: subprocess | ga-80po0c.2.1 | screen session-script reconciler-owned path contract is a checked Medium owner; the adapter subprocess factory is confined to TestScreenSessionScriptOwnedPathContract and each child uses isolated temporary state | P0.4b | 2026-10-01 |
...
| Source debt ratchet | all untagged test source | subprocess: 412 calls / 117 files (historical regex census: 380 / 98) | ga-80po0c.2 | untagged subprocess call/file totals cannot grow; reductions must lower this baseline; each process-owning test removes or replaces its source call site | D1/D2/D5/D6/E6 | 2026-10-01 |
