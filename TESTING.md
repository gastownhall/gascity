...
| Audit baseline | all tracked test source | subprocess: 637 calls / 185 files (historical regex census: 495 / 135) | ga-80po0c.2 | tracked test source totals remain visible as audit evidence; ga-80po0c.2 owns this point-in-time source census | P0.4a | 2026-10-01 |
...
| Medium owner | `internal/worker/adapters/zcode` package `zcode_test` | TestZcodeReplDrainGuardToleratesUnsetMore: subprocess | ga-uqdim3 | zcode-repl drain-guard unset-variable regression proof is a checked Medium subprocess owner; the one bash subprocess is confined to TestZcodeReplDrainGuardToleratesUnsetMore, which exists to prove the embedded zcode-repl script's drain-loop guard survives an unset $more under set -u -- the macOS/bash-3.2 divergence (ga-uqdim3) can't be reproduced by timing out a real read on this host's bash 5, so the hazard is forced directly and evaluated by a real bash interpreter | P0.4b | 2026-10-01 |
...
| Source debt ratchet | all untagged test source | subprocess: 430 calls / 126 files (historical regex census: 380 / 98) | ga-80po0c.2 | untagged subprocess call/file totals cannot grow; reductions must lower this baseline; each process-owning test removes or replaces its source call site | D1/D2/D5/D6/E6 | 2026-10-01 |
