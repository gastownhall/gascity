...
|---|-----------|--------|----------|
| 1 | Acceptance criteria met | PASS | `internal/beads/sqlite_store.go` and `sqlite_store_test.go` are deleted. Hard-error switch is present at `cmd/gc/main.go:1182` for `"sqlite"`, `"sqlite-cgo"`, and `"coordstore"` provider names. Orphaned test helpers in `cmd/gc/bead_policy_store_test.go` and `cmd/gc/beads_provider_lifecycle_test.go` are pruned. |
