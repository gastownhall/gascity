...
| 2 | Acceptance criteria met | PASS | `TestPinnedIntegrationBeadsModuleVersion` no longer embeds the stale `v1.3.0-rc.2` literal. It parses the repository's own `go.mod` require directive with `modfile.Parse` and compares that declaration with the independently resolved `go list -m` version, catching the declared pin drifting from the version MVS actually selects. A replacement is invisible to that comparison — `go list -m -f '{{.Version}}'` reports the require-directive version even under a `replace` — so a separate assertion reads `.Replace` and fails when any replacement is in effect. The exact test passes, so the current `v1.3.0` pin is accepted and future reviewed pin changes cannot silently leave a second version literal stale. |
...
| 4 | No high-severity review findings open | PASS | The exact-head review records no style or security blocker and no unresolved HIGH finding. Its only note concerned the `golang.org/x/mod` dependency; that module is already a direct require at `go.mod:37` and is imported by production code in `internal/beads/contract/preflight_checker.go`, so no dependency change was needed. No dependency version or checksum changed. |
...
- The existing test compares that declaration against the independently resolved module version, detecting declared-vs-selected drift while automatically following a reviewed pin bump.
- `beadsModuleReplacement` separately asserts no `replace` directive is in effect for `github.com/steveyegge/beads`, since the version comparison alone cannot see one.
