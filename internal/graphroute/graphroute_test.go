...
func TestApplyGraphRouting_OrderDispatchNilAgent_RigScopedOneShot_IndependentSteps(t *testing.T) {
	...
	newRecipe := func() *formula.Recipe {
		return &formula.Recipe{
			...
			Steps: []formula.RecipeStep{
				...
				// No continuation group is seeded here on purpose, matching
				// TestApplyGraphRouting_OneShotPoolLeavesExecutableStepsIndependent:
				// under #6360 a formula-declared group is propagated (or refused
				// loudly at the leaf), never silently dropped, so independence
				// for a one-shot step means "the step declared nothing and
	...
	assertIndependent := func(t *testing.T, recipe *formula.Recipe) {
		t.Helper()
		work := recipe.Steps[1]
		// gc.routed_to is the proof that resolution succeeded: pre-fix the
		// qualified-only resolver misses and ApplyGraphRouting returns before
		// stamping anything.
		if got := work.Metadata[beadmeta.RoutedToMetadataKey]; got != "rig1/oneshot" {
			t.Fatalf("gc.routed_to = %q, want rig1/oneshot", got)
		}
		if got := work.Metadata[beadmeta.ContinuationGroupMetadataKey]; got != "" {
