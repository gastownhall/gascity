...
func TestBootstrapPolicyOwnsNetListenDebtAndExactMediumOwners(t *testing.T) {
	...
	if debt.BaselineCalls != 100 || debt.BaselineFiles != 39 || debt.ReportedCalls != 92 || debt.ReportedFiles != 34 {
		t.Fatalf("stream-listener source baseline/reported = %d/%d, %d/%d; want 100/39, 92/34", debt.BaselineCalls, debt.BaselineFiles, debt.ReportedCalls, debt.ReportedFiles)
	}
	smallDebt := findRow(t, bootstrapPolicy.SmallDebt, ScopeUntagged, ResourceNetListen)
	if smallDebt.BaselineCalls != 98 || smallDebt.BaselineFiles != 38 {
		t.Fatalf("stream-listener Small baseline = %d/%d, want 98/38", smallDebt.BaselineCalls, smallDebt.BaselineFiles)
	}
