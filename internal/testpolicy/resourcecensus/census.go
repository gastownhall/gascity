...
var bootstrapPolicy = Ledger{
	...
	Medium: []MediumOwner{
		...
		{
			PackageDir:      "internal/worker/adapters/zcode",
			PackageName:     "zcode_test",
			Owner:           "TestZcodeReplDrainGuardToleratesUnsetMore",
			Resources:       []Resource{ResourceSubprocess},
			OwnerBead:       "ga-uqdim3",
			Invariant:       "zcode-repl drain-guard unset-variable regression proof is a checked Medium subprocess owner",
			ResourceOwner:   "the one bash subprocess is confined to TestZcodeReplDrainGuardToleratesUnsetMore, which exists to prove the embedded zcode-repl script's drain-loop guard survives an unset $more under set -u -- the macOS/bash-3.2 divergence (ga-uqdim3) can't be reproduced by timing out a real read on this host's bash 5, so the hazard is forced directly and evaluated by a real bash interpreter",
			MigrationTarget: "P0.4b",
			Expires:         "2026-10-01",
		},
	},
