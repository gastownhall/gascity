...
var bootstrapPolicy = Ledger{
	...
	Medium: []MediumOwner{
		...
		},
		{
			...
			PackageName:     "hooks",
			Owner:           "TestReconcileCodexHooksWaitsForCrossProcessHookSave",
			Resources:       []Resource{ResourceSubprocess},
			OwnerBead:       "ga-80po0c.2.1",
			Invariant:       "Codex hook cross-process path-lock regression is a checked Medium owner",
			ResourceOwner:   "the test-binary helper subprocess is confined to TestReconcileCodexHooksWaitsForCrossProcessHookSave, signals readiness over stdout, and is released over stdin",
			MigrationTarget: "P0.4b",
			Expires:         "2026-10-01",
		},
		{
			PackageDir:      "internal/runtime/exec",
			PackageName:     "exec",
			Owner:           "TestScreenSessionScriptOwnedPathContract",
			Resources:       []Resource{ResourceSubprocess},
			OwnerBead:       "ga-80po0c.2.1",
			Invariant:       "screen session-script reconciler-owned path contract is a checked Medium owner",
			ResourceOwner:   "the adapter subprocess factory is confined to TestScreenSessionScriptOwnedPathContract and each child uses isolated temporary state",
			MigrationTarget: "P0.4b",
			Expires:         "2026-10-01",
		},
		{
