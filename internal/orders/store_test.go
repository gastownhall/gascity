...
func TestLastRunAndCursorReads(t *testing.T) {
	rec := beadstest.NewRecordingStore(beads.NewMemStore())
	st := NewStore(beads.OrdersStore{Store: rec})

	if _, err := st.CreateRun("rig/agent", RunOpts{}); err != nil {
		t.Fatalf("CreateRun: %v", err)
	}

	ts, ok, err := st.LastRun("rig/agent")
	if err != nil {
		t.Fatalf("LastRun: %v", err)
	}
	if !ok || ts.IsZero() {
		t.Errorf("LastRun = (%v, %v), want a non-zero cooldown clock", ts, ok)
	}

	cur, err := st.EventCursor("rig/agent")
	if err != nil {
		t.Fatalf("EventCursor: %v", err)
	}
	if cur != 0 {
		t.Errorf("EventCursor = %d, want 0 (no seq label on a fresh run)", cur)
	}
}
