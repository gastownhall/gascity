...
func TestSingleStoreWorkQueryKeepsItsFallThrough(t *testing.T) {
	a := &Agent{Name: "worker"}
	res := runGeneratedQuery(t, a.EffectiveWorkQueryFor(singleStoreTopology()), map[string]string{
		"GC_SESSION_ORIGIN": "ephemeral",
	}, `#!/bin/sh
...
`)
	if res.exit != 0 {
		t.Fatalf("the single-store work_query exited %d over a failing bd; a non-relocated city must behave exactly as it does today", res.exit)
	}
...
func runGeneratedQuery(t *testing.T, command string, env map[string]string, gcScript string) generatedQueryResult {
	...
	writeFakeBin(t, filepath.Join(tmp, "bd"), "#!/bin/sh\nprintf '[]'\n")
	...
	cmd.Stderr = &stderr
	err := cmd.Run()
	res := generatedQueryResult{stdout: stdout.String(), stderr: stderr.String()}
	if err != nil {
