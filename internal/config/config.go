...
func ValidateNamedSessions(cfg *City) error {
	...
	for i := range cfg.NamedSessions {
		...
		if strings.TrimSpace(agent.Namepool) != "" || len(agent.NamepoolNames) > 0 ||
			(agent.MaxActiveSessions != nil && *agent.MaxActiveSessions != 1) {
			return fmt.Errorf(
				"named_session %q: template %q is a pool agent (max_active_sessions != 1 or namepool set); named sessions require non-pool templates",
				s.QualifiedName(), agent.QualifiedName(),
			)
		}
