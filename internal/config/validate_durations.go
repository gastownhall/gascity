...
func ValidateDurations(cfg *City, source string) []string {
	...
	check("[daemon]", "dolt_lock_release_timeout", cfg.Daemon.DoltLockReleaseTimeout)
...
func ValidateNonNegativeDurations(cfg *City, source string) error {
	...
	if err := checkNonNegative("[daemon]", "dolt_lock_release_timeout", cfg.Daemon.DoltLockReleaseTimeout); err != nil {
		return err
	}
	for name, policy := range cfg.Beads.Policies {
