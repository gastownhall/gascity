...
func TestEffectiveWorkQueryDefault(t *testing.T) {
	...
	if !strings.Contains(got, `bd ready --metadata-field "gc.routed_to=$target" --unassigned --exclude-type=epic --exclude-label "hold:mayor" --exclude-label "hold:external" --json --sort priority --limit=20`) {
	...
	if !strings.Contains(got, `bd ready --metadata-field "gc.run_target=$target" --metadata-field "gc.kind=workflow" --unassigned --exclude-type=epic --exclude-label "hold:mayor" --exclude-label "hold:external" --json --sort priority --limit=20`) {
...
func TestEffectiveWorkQueryBD105CompatibilityOptIn(t *testing.T) {
	...
	if !strings.Contains(got, `bd ready --include-ephemeral --metadata-field "gc.routed_to=$target" --unassigned --exclude-type=epic --exclude-label "hold:mayor" --exclude-label "hold:external" --json --sort priority --limit=20`) {
...
set -eu
case "$*" in
  *"ready --include-ephemeral"*"--metadata-field gc.routed_to=gascity/control-dispatcher"*"--unassigned"*"--exclude-type=epic"*"--json"*"--sort priority"*"--limit=20"*)
    printf '[]'
    ;;
  *"ready --metadata-field gc.routed_to=gascity/control-dispatcher"*"--unassigned"*"--exclude-type=epic"*"--json"*"--sort priority"*"--limit=20"*)
    printf '[]'
    ;;
  *"ready --metadata-field gc.routed_to=gascity/workflow-control"*"--unassigned"*"--exclude-type=epic"*"--json"*"--sort priority"*"--limit=20"*)
...
func TestEffectiveWorkQueryRoutedQueueRequestsNativePrioritySortAcrossReadyTiers(t *testing.T) {
	a := Agent{Name: "worker", Dir: "hello-world"}
	got := a.EffectiveWorkQuery()
	for _, want := range []string{
...
set -eu
case "$*" in
  "ready --metadata-field gc.routed_to=hello-world/worker --unassigned --exclude-type=epic --exclude-label hold:mayor --exclude-label hold:external --json --sort priority --limit=20")
...
`)
	if !strings.Contains(out, "older-no-history") {
		t.Fatalf("EffectiveWorkQuery() did not return the native priority-sort fixture: %q", out)
	}
	if strings.Contains(out, "newer-durable") {
		t.Fatalf("EffectiveWorkQuery() returned more than the first priority-ordered routed work: %q", out)
	}
}
...
func TestEffectiveWorkQueryRoutedQueueEmitsPrioritySort(t *testing.T) {
	a := Agent{Name: "worker", Dir: "hello-world"}
	out := runEffectiveWorkQuery(t, a, map[string]string{
		"GC_SESSION_ORIGIN": "ephemeral",
	}, `#!/bin/sh
set -eu
case "$*" in
  *"ready --metadata-field gc.routed_to=hello-world/worker"*"--unassigned"*"--exclude-type=epic"*"--json"*"--sort priority"*"--limit=20"*)
...
`)
	if !strings.Contains(out, "older-p2") {
		t.Fatalf("EffectiveWorkQuery() did not return the --sort priority fixture: %q", out)
	}
...
func TestEffectiveWorkQueryRoutedFallbackRequestsNativePrioritySort(t *testing.T) {
	a := Agent{Name: "worker", Dir: "hello-world"}
	out := runEffectiveWorkQuery(t, a, map[string]string{
		"GC_SESSION_ORIGIN": "ephemeral",
	}, `#!/bin/sh
...
case "$*" in
  *"ready --metadata-field gc.routed_to=hello-world/worker"*"--unassigned"*"--exclude-type=epic"*"--json"*"--sort priority"*"--limit=20"*)
    printf '[]'
    ;;
  *"ready --metadata-field gc.run_target=hello-world/worker"*"--metadata-field gc.kind=workflow"*"--unassigned"*"--exclude-type=epic"*"--json"*"--sort priority"*"--limit=20"*)
...
`)
	if !strings.Contains(out, "older-fallback") {
		t.Fatalf("EffectiveWorkQuery() did not return the fallback --sort priority fixture: %q", out)
	}
	if strings.Contains(out, "newer-fallback") {
		t.Fatalf("EffectiveWorkQuery() returned more than the first priority-ordered fallback work: %q", out)
	}
}
...
func TestPoolDemandPredicateSharedWithWorkQuery(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			workPredicate := bdReadyPoolDemandShell("--sort priority --limit=20", QueryTopology{})
