...
| SESSION-RECON-015 | Agent API session origin | Both configured-agent API projections persist `session_origin=ephemeral`, including generated adhoc identities. Their default hook may claim unassigned work routed to the base template. Provider/manual and configured named sessions retain their own origins and cannot claim that shared demand. API creation does not imply controller pool ownership. | issue #5009; `internal/api/handler_sessions_test.go` (`TestHumaHandleSessionCreateUsesACPTransportCommandForAgentTemplate`); `cmd/gc/cmd_hook_test.go` (`TestCmdHookPoolDemandOriginGate`) |
