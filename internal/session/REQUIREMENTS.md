...
| SESSION-ID-012 | Named marker repair preserves ownership | A transient desired-state classification may restore a configured named session's markers only when the exact runtime-name bead still matches that session's backing template. A foreign pool bead using the reserved runtime name remains a conflict and is not promoted. | `cmd/gc/session_beads_test.go` (`TestSyncSessionBeads_HealsConfiguredNamedMarkersWhenPoolTickReusesOwnerName`, `TestSyncSessionBeads_DoesNotPromoteForeignPoolBeadWithReservedRuntimeName`); ga-3zs.1 |
