...
| SESSION-RUNTIME-011 | Fast input acceptance | If runtime delivery cannot confirm acceptance, one new complete native user record may confirm that input under the same delivery lock. Missing, stale, partial, ambiguous, or changed history preserves the original error; confirmation never resends input. | `internal/session/submit_confirmation_test.go`; `internal/worker/message_receipt_test.go` |
