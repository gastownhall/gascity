...
| SESSION-RUNTIME-009 | Lifecycle runtime-observation uncertainty defers absence-consuming actions | Worker/session runtime observation propagates an error-bearing provider liveness failure instead of projecting it as stopped. Lifecycle callers treat liveness uncertainty as unknown and defer close, absence-derived heal metadata, pending-create rollback, start or zombie recycle, confirm-dead re-kill, drain completion (including the post-stop confirmation probe), and pool sweep until a later healthy observation. This includes secondary liveness observations after `ErrStateSync`, `ErrSessionExists`, and preserved-named classification: `ErrRuntimeUnavailable` defers before rollback, provider/rate-limit failure bookkeeping, close/heal, or wake-budget consumption. T3 `ErrRuntimeUnavailable` from a later attachment or last-activity observation is likewise preserved while unrelated legacy-provider activity errors remain best-effort. After primary liveness succeeds, that secondary uncertainty fences only its owning attachment/activity-derived transitions: no detached-at write, awake-input no-wake or idle drain, config-drift restart, or `Stop`; ready idle-probe, drain, wake-budget, and resume metadata stay intact. Earlier positive-liveness bookkeeping remains authoritative and is not rolled back. Positive control-plane evidence still updates non-durable bookkeeping: a configured named spec resets its consecutive-absence confirmation window even on a liveness-error tick, without mutating bead metadata. A deferred start releases only its in-flight wake lease: the bead stays open, any pending-create claim and resume metadata remain intact, and no wake-failure budget is consumed. Confirmed absence and legacy providers retain their existing behavior. | `internal/session/manager.go`; `internal/session/manager_test.go` `TestObserveRuntimeForInfoPreservesOnlyRuntimeUnavailableActivityErrors`; `internal/worker/observe.go`; `internal/worker/runtime_handle.go`; `internal/worker/handle_test.go` `TestRuntimeHandleLiveObservationPreservesOnlyRuntimeUnavailableActivityErrors`; `internal/runtime/t3bridge/provider_test.go` `TestSeamBackedLastActivityPreservesSnapshotUncertainty`; `cmd/gc/session_lifecycle_parallel.go`; `cmd/gc/session_reconciler.go`; `cmd/gc/session_reconciler_observation_deferral_test.go` `TestReconcileSessionBeads_AttachmentUnavailableDoesNotRecordDetachedAt`, `TestReconcileSessionBeads_AwakeAttachmentUnavailableDoesNotStartNoWakeDrain`, `TestReconcileSessionBeads_ActivityUnavailableDoesNotStartIdleDrain`, `TestReconcileSessionBeads_ConfigDriftActivityUnavailableDoesNotRestartNamedSession`, `TestReconcileSessionBeads_ConfigDriftDrainAckRuntimeUnavailablePreservesDrainState`; `cmd/gc/session_sleep_test.go` `TestShouldBeginIdleDrainInfoRuntimeUnavailableRetainsReadyProbe`; `cmd/gc/session_reconciler_trace_test.go` `TestReconcileOrphanCloseFailsClosedOnLivenessError`; `cmd/gc/session_reconciler_named_collapse_test.go` `TestReconcileSessionBeads_NamedSpecReappearsDuringLivenessErrorClearsDeferral`; `cmd/gc/session_lifecycle_start_boundary_test.go` `TestStartPreparedStartCandidateDefersWhenLivenessUnavailable`, `TestExecutePreparedStartWaveDefersLivenessUnavailableWithoutLifecycleFailure`, `TestExecutePreparedStartWaveDefersErrStateSyncRecoveryWhenObservationUnavailable`, `TestExecutePreparedStartWaveDefersErrSessionExistsRecoveryWhenObservationUnavailable`; `cmd/gc/session_reconciler_test.go` `TestReconcileSessionBeads_PreservedNamedSecondaryLivenessErrorDefersLifecycle`; `cmd/gc/session_wake_test.go` `TestAdvanceSessionDrains_LivenessUnavailableDefersOrdinaryDrainCompletion`, `TestAdvanceSessionDrains_LivenessUnavailableAfterVerifiedStopDefersCompletion`; `cmd/gc/city_runtime_test.go` `TestSweepUndesiredPoolSessionBeads_DefersWhenLivenessUnavailable` |
