...
| SESSION-ID-012 | Closed drained named-name holder | A closed lifecycle record that retains a configured named session's runtime name does not block a fresh canonical successor. The historical bead remains closed and unchanged. | `internal/api/session_model_phase0_lifecycle_spec_test.go` (`TestPhase0HandleSessionWake_NamedIdentityIgnoresClosedDrainedNameHolder`) |
