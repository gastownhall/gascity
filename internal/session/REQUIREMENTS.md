...
| SESSION-RUNTIME-009 | Named tmux attach no-start fence | Interactive, direct, and hidden `attach-session` paths against a configured per-city tmux socket use tmux `-N` for observational checks, bounded preflight, and final attach. Those commands never start or rebind a server when the socket is missing or disappears; preflight no-server observations fail closed with exact-path diagnostics. Every attach launch is fenced by a socket witness captured beforehand, so a socket replaced between preflight and attach is refused rather than adopted. Default-socket behavior is unchanged. | `internal/runtime/tmux/adapter_unit_test.go`; `internal/runtime/tmux/tmux_test.go`; `internal/runtime/tmux/server_probe_test.go` |
