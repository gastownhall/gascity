...
| SESSION-RECON-017 | Bead-scoped pool create rollback | A failed pool create whose runtime name embeds its bead ID stops that runtime before closing the row. A failed stop leaves the pending-create lease open so a later reconciliation can retry; after confirmed teardown, terminal-provider diagnostics persist and the fenced rollback closes the row as failed-create before a successor is minted. | `cmd/gc/pool_session_name_churn_test.go` (`TestPoolSessionCreate_FailedCreatesLeaveAtMostOneLiveRuntime`, `TestPoolSessionCreate_TeardownFailureHoldsTheSlot`, `TestPoolSessionCreate_TerminalProviderErrorTearsDownBeforeRollback`) |
