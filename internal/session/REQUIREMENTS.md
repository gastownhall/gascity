...
| SESSION-START-010 | Suspend preserves conversation continuity | Successful runtime suspension records deliberate sleep with a durable hold and clears the prior wake attempt. Repeated controller ticks must keep the session dormant despite ordinary demand; successful explicit manager resume releases the suspension hold, while failed resume and runtime-only operations preserve it. Subsequent controller state healing and exit classification must neither count a crash/churn event nor discard the provider session key, including suspension inside the rapid-crash and churn windows. In-process API/controller coordination must preserve newer holds and defer stale reconciliation or wake selection; a later explicit wake remains valid. | `cmd/gc/session_suspend_continuity_test.go` (`TestSuspendPreservesConversationThroughExitReconciliation`); `cmd/gc/session_suspend_policy_test.go` (`TestManagerSuspendRemainsDormantAcrossControllerTicks`); `internal/session/suspend_hold_test.go`; `cmd/gc/session_suspend_overlap_test.go` (`TestReconcileSessionBeadsDoesNotUndoConcurrentSuspend`, `TestCurrentSessionExitSerializesSuspendAcrossWholeDecision`, `TestCurrentSessionExitBypassesStaleCachingStore`); `cmd/gc/session_start_suspend_overlap_test.go` (`TestSelectedStartCandidateCannotUndoLaterSuspend`, `TestSelectedStartCandidateAllowsNewExplicitWakeAfterSuspend`); `internal/session/manager.go` (`Suspend`) |
| SESSION-START-011 | Configured transcript roots preserve resume identity | Launch preflight, fork validation and wake-failure recovery use the same configured transcript roots as history discovery. A present keyed conversation keeps its identity after a transient launch failure or rapid exit; an absent or unprobeable conversation retains the existing recovery policy. | `cmd/gc/session_resume_observe_paths_test.go`; `cmd/gc/session_reconcile_test.go` (`TestWakeFailureUsesConfiguredTranscriptRoots`) |
| SESSION-START-012 | Explicit singleton API session starts | An explicitly created agent session with a pending create claim and no controller pool markers must start under its existing session identity even when `max_active_sessions=1` and automatic demand is zero. Controller-marked capacity remains demand-controlled. Explicit sessions retain the existing manual-session soft-cap policy. | `cmd/gc/session_api_singleton_start_test.go` (`TestAPIRequestedSessionStartsWithoutPoolDemand`); `engdocs/design/session-model-unification.md` (Cap Accounting) |
...
| SESSION-RUNTIME-010 | Input after Codex resume | When a submit must resume a non-ACP Codex runtime, require current runtime input readiness before nudging. Historical idle transcript state and prior startup-dialog verification are insufficient. Readiness timeout, cancellation, or unsupported observation fails without sending the message; already-running Codex retains immediate default delivery. | `internal/session/codex_resume_ready_test.go`; `internal/session/submit_test.go`; `internal/runtime/tmux/codex_ready_test.go` |
| SESSION-RUNTIME-011 | Fast input acceptance | If runtime delivery cannot confirm acceptance, one new complete native user record may confirm that input under the same delivery lock. Confirmation may wait briefly for delayed publication within the caller's deadline. Cancellation, missing, stale, partial, ambiguous, or changed history preserves the original error; confirmation never resends input. | `internal/session/submit_confirmation_test.go`; `internal/worker/message_receipt_test.go` |
