...
| SESSION-START-009 | Same-host start transaction | Controller prepare, provider start, sync/async refresh, commit metadata, and `session.woke` emission serialize with suspend/close through one per-session lifecycle lock. Fresh validation includes the exact instance token, session name, and continuation epoch. A later suspend/close wins without a stale commit overwrite or wake event. | `cmd/gc/session_lifecycle_parallel_test.go` (`TestCommitStartResultAndSuspendHaveOneLifecycleOrder`, `TestStartLifecycleLeaseIncludesSessionNameAndContinuationEpoch`) |
| SESSION-START-010 | Destructive runtime identity fence | Zombie recycle and stale async cleanup stop only a runtime presenting the exact durable session ID, session name, instance token, and continuation epoch; a present runtime epoch must also match. A newer runtime that reuses the same bead ID and name survives stale cleanup. | `cmd/gc/session_lifecycle_parallel_test.go` (`TestExecutePreparedStartWave_DoesNotRecycleNewerSameIDZombie`, `TestStopStaleAsyncStartRuntimeRequiresExactIncarnation`, `TestRuntimeIdentityFenceIncludesSessionNameAndContinuationEpoch`) |
| SESSION-START-011 | Visible create can be superseded | After a started-create obtains and exposes its bead ID, lifecycle protection and a fresh read precede runtime start. If suspend or close wins first, create returns `ErrCreateSuperseded`, performs no runtime start, and preserves the winning state. | `internal/session/manager_test.go` (`TestCreateStartedAbortsWhenVisibleBeadIsSuperseded`) |
| SESSION-START-012 | Delayed wake is exact and bounded | Every explicit wake has an unambiguous token independent of its timestamp; the request, timestamp, and token are projected through `Info` for lifecycle fencing. CLI normalization and delayed API start act only on that exact still-current token; later suspend wins. API provider start is bounded, releases lifecycle ownership on cancellation, and successful already-running convergence consumes the wake request. | `internal/session/info_apply_patch_test.go`; `internal/session/wait_store_test.go`; `cmd/gc/cmd_session_wake_test.go` (`TestDoSessionWakeNormalizationCannotOverrideLaterSuspend`); `internal/api/handler_sessions_test.go` (`TestHandleSessionWakeAlreadyRunningConsumesRequestToken`, `TestHandleSessionWakeDelayedStartCannotOverrideSuspend`, `TestHandleSessionWakeStartTimeoutReleasesLifecycleLock`) |
...
| SESSION-WORK-005 | Drain completion intent ownership | Completing an ordinary assigned-work drain consumes controller-owned `idle-stop-pending`; an explicit operator `user-hold` remains durable until explicit wake. | `cmd/gc/session_reconciler_test.go` (`TestFinalizeDrainAckStoppedSessionClearsOnlyControllerIdleIntent`) |
| SESSION-WORK-006 | Direct drain-ack claim coherence | If a legacy direct-ack path durably claims stop-pending and a later read/write fails, the caller snapshot folds the partial claim. A subsequent close folds from that claimed base and exactly matches persisted state. | `cmd/gc/session_reconciler_test.go` (`TestFinalizeDrainAckStoppedSessionDirectClaimFailureFoldsDurableClaim`, `TestFinalizeDrainAckStoppedSessionDirectClaimCloseFoldIsExact`) |
| SESSION-WORK-007 | Drain finalization incarnation fence | Stale user-hold and explicit-wake exceptions are finalizable only with matching immutable generation, instance token, and awake epoch evidence. A superseded observer adopts the authoritative row without clearing newer provider or in-memory drain state; a closed-bead witness clears only bead-ID tracker state because its runtime name may already belong to a replacement. | `cmd/gc/session_reconciler_test.go` (`TestFinalizeDrainAckStoppedSessionVersionExceptionsRequireSameIncarnation`, `TestFinalizeDrainAckStoppedSessionSupersededWitnessPreservesNewDrainState`, `TestFinalizeDrainAckStoppedSessionClosedWitnessPreservesReusedRuntimeDrain`) |
| SESSION-WORK-008 | Busy drain finalizer is a no-op | A stop-pending sweep that cannot acquire the nonblocking lifecycle lock neither reports a finalization nor requests a snapshot reload. | `cmd/gc/session_reconciler_test.go` (`TestFinalizeDrainAckStopPendingSessionsBusyLifecycleDoesNotSignalReload`) |
| SESSION-WORK-009 | Provider drain cleanup precedes name release | Name-scoped provider drain metadata is cleared while the terminal bead still owns the runtime name, before either closing the bead or publishing completed-drain metadata. Cleanup failure leaves the bead stop-pending/open and its bead-ID tracker intact for retry; a replacement that starts immediately after close keeps its own markers. | `cmd/gc/session_reconciler_test.go` (`TestFinalizeDrainAckStoppedSessionClearsProviderDrainBeforeClosing`, `TestFinalizeDrainAckStoppedSessionProviderClearFailureLeavesAuthorityForRetry`) |
| SESSION-WORK-010 | Exact, retryable drain finalization | Drain-finalizer exceptions require positive incarnation proof plus an exact canonical operator-park or explicit-wake delta across the full projected lifecycle identity, including runtime name, continuation epoch, and wake request timestamp/token. Mark, direct claim, failed-close restoration, and final completion each use one backend `Update`. An assignment race completes asleep; a failed close transaction restores stop-pending without clearing its tracker or emitting terminal events, and a later retry closes. A transaction error whose close nevertheless became visible is adopted as a terminal witness. Terminal close time is sampled once for both persistence and snapshot fold. A closed/superseded witness cannot clear or later advance a replacement incarnation's lifecycle-pinned tracker or name-scoped provider metadata. | `cmd/gc/session_reconciler_test.go` (`TestFinalizeDrainAckStoppedSessionVersionExceptionsRejectUnmodeledDrift`, `TestFinalizeDrainAckStoppedSessionAcceptsExactExplicitWakeDelta`, `TestExplicitWakePreservesDrainStopIdentityRejectsNonCanonicalCoordinates`, `TestDrainAckLifecycleClustersUseSingleUpdate`, `TestFinalizeDrainAckStoppedSessionCloseFailureIsRetryable`, `TestFinalizeDrainAckStoppedSessionAdoptsAmbiguousSuccessfulClose`, `TestFinalizeDrainAckStoppedSessionSamplesCloseTimeOnce`, `TestFinalizeDrainAckStoppedSessionFallsThroughWhenCloseGateRacesWithAssignment`, `TestFinalizeDrainAckStoppedSessionClosedWitnessThenAdvancePreservesReplacementDrainLifecycle`) |
| SESSION-WORK-011 | Controller start preserves fresh drain acknowledgement | Bead-backed controller starts execute under the exact lifecycle fence and delegate predecessor acknowledgement cleanup to the Session Manager's final pre-launch boundary. They preserve explicit `GC_DRAIN`, fail closed when cleanup is ambiguous, perform no name-scoped acknowledgement clear after launch, and therefore preserve an agent acknowledgement written before `Provider.Start` returns through both synchronous and asynchronous commit. | `cmd/gc/session_lifecycle_parallel_test.go` (`TestExecutePlannedStartsClearsLegacyDrainAckBeforeProviderStartAndPreservesDrain`, `TestExecutePlannedStartsPreservesFreshAgentDrainAckWrittenBeforeStartReturns`, `TestExecutePlannedStartsDefersWhenStaleDrainAckCannotBeCleared`) |
| SESSION-WORK-012 | Session Manager replacement clears predecessor acknowledgement | Every Session Manager provider launch first confirms escaped predecessors dead, then removes the five `GC_DRAIN_ACK` acknowledgement/provenance keys immediately before `Provider.Start`, including create, resume, runtime-only, and failed-start retry paths. `GC_DRAIN` is preserved. Session-gone removal is already clean; every other cleanup error prevents launch. No post-start clear may erase an acknowledgement written by the replacement before `Start` returns. | `internal/runtime/drain_ack_test.go`; `internal/session/manager_test.go` (`TestCreateClearsOrphanDrainAckImmediatelyBeforeReplacementStart`, `TestCreateDrainAckCleanupErrorPreventsRuntimeStart`, `TestStartRetryClearsFailedRuntimeAckAndPreservesFreshRuntimeAck`, `TestRuntimeStartCallSitesUsePreparationGate`) |
| SESSION-WORK-013 | Stalled-execution drain authority and convergence | A bounded stalled-execution escalation belongs to one exact full session lifecycle version and runtime identity (session ID, `generation`, `instance_token`, `awake_started_at`, plus available runtime/continuation epochs) and one exact work ownership incarnation (store, ID, assignee, revision and/or claim fence). Enqueue, stop, and terminal close serialize through the city/session lifecycle lock and revalidate live session plus work authority; changed authority retires the drain and unavailable/busy authority holds without signaling or stopping. A confirmed stopped seat closes terminally rather than remaining wakeable asleep, same-store work releases immediately, split-store work releases through the next orphan pass, and partial close failure restores the open lifecycle for retry. Controller restart reconstructs the same guarded drain idempotently without replaying its event or nudge budget. | `cmd/gc/execution_backstop_test.go`; `cmd/gc/execution_stalled_convergence_test.go`; `internal/beads/native_dolt_store_test.go` |
| SESSION-WORK-014 | Distributed hook-activity gate and SessionStart receipt handoff | Remote hooks (K8s/SSH) share the bead store but not the controller's host-local lifecycle flock, so hook output is ordered against the terminal stalled fence through a single-key metadata CAS gate (`session_hook_activity_gate`, value-CAS over canonical struct JSON; kinds hook/hook-released/receipt-consume/execution-stalled). A hook lease is acquired from the runtime's exact identity coordinates before any shared-store output and released into a durable tombstone that only a proven provider boundary (stopped runtime, or released-kind plus idle-grace quiet) may acknowledge; missing CAS support is a hard refusal, never an unconditional fallback. The controller's stalled fence installs inside the destructive latch boundary and defers on any foreign occupant; a released tombstone for the exact coordinates is the hook's completion announcement and may be cleared by the receipt-consume fence. `gc prime --hook` never mutates the session row: it publishes an append-only receipt bead validated against the live launch authority, and the controller consumes it into `session_key` only under the deterministic receipt-consume fence with full live-authority revalidation; an absent `continuation_epoch` normalizes to the first epoch on both sides of every coordinate match, mirroring the runtime identity fence. The claim fence refuses a session holding the latch, the gate's stalled kind, or the fence-installed-before-latch crash window in one read. | `internal/session/hook_activity_gate_test.go`; `internal/session/provider_session_key_receipt_test.go`; `cmd/gc/execution_stalled_fence_wiring_test.go`; `cmd/gc/prime_session_key_capture_test.go`; `cmd/gc/main_test.go` (`TestDoPrime*HookPersists*`) |
...
| SESSION-RUNTIME-009 | Resume command template parity | CLI resume/attach and API wake reconstruct the canonical session setup context and expand a configured start command before comparing it with persisted command state or starting the replacement runtime. Rig-qualified providerless templates expand with their qualified identity without requiring a configured provider; the persisted provider remains empty, while worker normalization may derive an executable label for the runtime boundary. | `cmd/gc/worker_handle_test.go` (`TestResolvedWorkerRuntimeExpandsRigQualifiedProviderlessStartCommand`, `TestWorkerHandleStartExpandsRigQualifiedProviderlessStartCommand`); `internal/api/handler_sessions_test.go` (`TestHandleSessionWakeExpandsRigQualifiedProviderlessStartCommand`) |
