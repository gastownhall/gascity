...
| SESSION-RUNTIME-009 | Provider-native remote checkpoint | A provider-native remote session has one versioned, content-free checkpoint in session-bead metadata under `gc.remote_session.v1`: opaque session/run identity, normalized phase/failure, event/receipt cursors, Git handoff metadata, and timestamp only. The writer holds the per-session mutation lock and must match the current `instance_token`; a stale session incarnation cannot overwrite the checkpoint. Unknown schema versions and invalid normalized state fail closed. Prompts, transcript events, credentials, provider messages, account/plan/billing data, and quota amounts are excluded. | `internal/session/remote_session.go`; `internal/session/remote_session_test.go`; `internal/runtime/remote.go` |
