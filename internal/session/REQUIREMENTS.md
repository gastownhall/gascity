...
| SESSION-RECON-015 | Retry a claimed step after explicit provider overload | A running pool session that holds exactly one in-progress claim and shows a high-confidence retryable provider-overload response re-delivers its own claim nudge on that reconciliation pass instead of waiting for the generic quiet-session grace. The attempt is durably reserved first; later retries retain the normal backoff and cap, and ordinary recent activity remains exempt. | `cmd/gc/execution_backstop.go`; `cmd/gc/execution_backstop_test.go` `TestExecutionBackstopRetriesAProviderOverloadWithoutGenericGrace` |
