...
/**
 * Post v0 city by city name session by ID reset
 *
 * Requests a fresh restart of an existing session: the session bead is kept, and the controller restarts the runtime on the next continuation epoch, so the provider starts a new conversation instead of resuming the previous one. The call records the request and pokes the controller; it returns before the restart happens. A closed session returns 409. Unlike `gc session reset`, this endpoint does not clear a tripped named-session respawn circuit breaker.
 */
export const postV0CityByCityNameSessionByIdReset = <ThrowOnError extends boolean = false>(options: Options<PostV0CityByCityNameSessionByIdResetData, ThrowOnError>) => (options.client ?? client).post<PostV0CityByCityNameSessionByIdResetResponses, PostV0CityByCityNameSessionByIdResetErrors, ThrowOnError>({ url: '/v0/city/{cityName}/session/{id}/reset', ...options });
