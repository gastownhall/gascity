...
type cityRunTailer struct {
	...
	// subMu guards the per-run detail-stream subscriber registry and the
	// monotonic publish generation. It is a distinct lock from mu so a stream
	// broadcast never contends with the hot fold-publish path's RLock/Lock; both
	// are taken only briefly and never across a network write. See
	// rundetail_stream.go.
	subMu      sync.Mutex
	generation uint64
...
func (t *cityRunTailer) build(proj *runproj.Projector, prevMarks map[string]runproj.LaneProgressMark, loadErr error) map[string]runproj.LaneProgressMark {
	...
	// place a detail-stream broadcast fires: bump the generation and notify every
	// subscriber (non-blocking). A subscriber that has not yet drained its prior
	// notify already has a rebuild pending, so a full buffer is a no-op — the
