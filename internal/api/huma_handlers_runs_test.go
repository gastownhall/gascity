...
func TestDeriveRunStatus(t *testing.T) {
	...
	cases := []struct {
		...
		{"closed-canceled", "complete", closedRoot("canceled"), true, 3, RunStatusCancelled},
		// Cancel: an open root carrying the intent marker reports cancelling.
		{"open-cancel-requested", "active", beads.Bead{Status: "open", Metadata: map[string]string{"gc.cancel_requested": "true"}}, true, 2, RunStatusCancelling},
...
func TestDeriveRunStepStatus(t *testing.T) {
	...
	cases := []struct {
		...
		{"closed-canceled", step("closed", "canceled"), RunStepStatusCancelled},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
