...
if [ -d "$CITY_BEADS_DIR" ]; then
    ...
    if [ -n "$SESSION_BEAD_PATTERN" ]; then
        ...
        # Backup-age gate: skip bulk prune unless the canonical state written
        # after every required Dolt database sync is recent.
        ...
        _BACKUP_STATE="${GC_DOLT_BACKUP_STATE_FILE:-$CITY_BEADS_DIR/dolt-backup-state.json}"
