...
// TestHybridSessionEventsMergeBothBackends pins that a composite whose
...
// TestHybridSessionEventsSingleBackendUnchanged pins that a composite with
// one event-capable backend returns that backend's stream as is.
func TestHybridSessionEventsSingleBackendUnchanged(t *testing.T) {
	b := newEventedBackend()
	p := New(runtime.NewFake(), b, isRemote)
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	ch, err := p.SubscribeSessionEvents(ctx)
	if err != nil {
...
// TestHybridSessionEventsNoBackendErrors pins the error when neither backend
// publishes session events.
func TestHybridSessionEventsNoBackendErrors(t *testing.T) {
