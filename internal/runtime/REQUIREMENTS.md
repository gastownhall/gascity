...
| RUNTIME-RPP-015 | Reconciler-owned mergeable-path staging | An exec runtime receives `reconciler_owned_mergeable_paths` only after declaring `staging.reconciler-owned-mergeable-paths`. For each exact path it must require one probed `copy_files` entry, re-hash its regular source immediately before staging, skip only that path in every overlay layer, copy the verified source to the exact workdir-relative destination, verify remote landed bytes before releasing the agent, and fail start/provision on any missing, duplicate, unsafe, changed, mismatched, or failed copy. `gc runtime check` runs positive, stale-digest, and missing-source fixtures against any runtime declaring the capability. Runtimes without the capability stay on legacy full-overlay staging. | `internal/runtime/exec/exec.go`; `internal/runtime/exec/json.go`; `internal/runtime/rppcheck/rppcheck.go`; `internal/runtime/rppcheck/rppcheck_test.go`; `internal/runtime/exec/session_script_test.go`; `contrib/session-scripts/gc-session-screen`; `contrib/session-scripts/gc-session-k8s`; `docs/reference/exec-session-provider.md` |
