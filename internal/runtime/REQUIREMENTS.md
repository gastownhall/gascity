...
| RUNTIME-CONTRACT-007 | Per-session liveness preserves observation uncertainty | The legacy `Provider.IsRunning(string) bool` and errorless `LivenessObserver` contracts remain compatible. Providers that can distinguish confirmed absence from failed observation expose the optional `LivenessObserverWithError` capability. Production T3 returns an error wrapping `ErrRuntimeUnavailable` when its snapshot observation fails outside startup grace. During startup grace, either a failed snapshot observation or a reachable binding whose materialized session status is `none`/`gone` remains provisional running+alive; only a reachable snapshot with no binding confirms absence and returns zero liveness with nil error. Auto and hybrid compositions forward the capability without folding backend errors into false. | `internal/runtime/liveness.go`; `internal/runtime/t3bridge/provider_test.go` `TestSeamBackedLivenessObservationPreservesSnapshotUncertainty`; `internal/runtime/auto/liveness_forwarding_test.go` `TestProvider_ForwardsLivenessObservationErrorFromRoutedBackend`; `internal/runtime/hybrid/hybrid_test.go` `TestProvider_ForwardsLivenessObservationErrorToRoutedBackend` |
