...
| RUNTIME-CONTRACT-006 | ACP pre-start ordering and cancellation | ACP stages overlays/copy files, then executes every `PreStart` command in order on the host before launching the ACP process. A failure or cancellation is fatal. Commands receive the resolved session environment and a deterministic existing cwd (`GC_DIR`, city root, WorkDir/ancestor, provider state, then temp). `setup_timeout` is the fixed per-command deadline by default; when `setup_max_timeout` is set it becomes the output-idle budget and the max timeout is the absolute ceiling, with cooperative process-group cancellation and rollback grace. A canceled startup keeps its name reserved until PreStart rollback/unwind completes, preventing overlapping setup against one workdir, and `Stop` is atomic with the final startup commit so cancellation cannot be lost at handoff. | `internal/runtime/acp/acp.go`; `internal/runtime/acp/prestart_test.go` (`TestStartStagesWorkDirBeforePreStart`, `TestStartPreStartCreatesWorkDirBeforeLaunch`, `TestRunPreStartSetupMaxTimeoutAllowsOutputProgress`, `TestRunPreStartSetupMaxTimeoutUsesIdleBudgetAndRunsRollback`, `TestRunPreStartSetupMaxTimeoutCapsContinuousOutput`, `TestStopDuringPreStartKeepsNameReservedUntilRollbackCompletes`); `internal/runtime/acp/prestart_handoff_test.go` (`TestStopAtStartupCommitCannotBeLost`); `cmd/gc/runtime_registry_test.go` (`TestRuntimeRegistryACPHonorsSessionSetupMaxTimeout`) |
