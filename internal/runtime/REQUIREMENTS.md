...
| RUNTIME-CONTRACT-007 | Retryable provider overload observation | The runtime helper recognizes the exact provider response `Selected model is at capacity. Please try a different model.` (including its TUI marker) as retryable overload evidence, while source code and ordinary discussion containing similar words do not match. | `internal/runtime/dialog.go`; `internal/runtime/dialog_test.go` `TestContainsProviderRetryableOverloadScreen` |
