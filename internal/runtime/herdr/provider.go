...
func (p *Provider) start(ctx context.Context, name string, cfg runtime.Config) error {
	...
	// Step 0: stage overlays/CopyFiles, then pre_start — workDir/worktree
	// preparation, and the carrier for stage-2 skill/MCP materialization.
	// Staging runs first, mirroring the other host-side providers (tmux
	// stageStartFiles, subprocess/acp StageSessionWorkDir): pre_start commands
	// may read staged files. Mirrors tmux doStartSession's first step; fatal on
	// failure so an agent never launches into an unprepared workDir. Runs only
	// once we know we're actually creating the agent (the ErrSessionExists
	// check above), so an existing session never re-runs prep.
	...
	// session_setup (Step 5.5), then deliver the first turn (Step 6).
	//
	...
	// Skip the whole block when we adopted an already-running holder: it is a
	// live, already-primed agent — re-delivering would inject the startup prime
	// into a working session, and session_setup is fresh-session preparation.
	startupText := startupDeliveryText(cfg)
	if info.PaneID != "" && !adopted && (startupText != "" || hasSessionSetup(cfg)) {
		...
		// (Step 5.5, after readiness, before the first turn). Non-fatal.
