...
func stubKillByPIDDeps(t *testing.T, infra func(int) bool, alive func(int) bool, kill func(int, syscall.Signal) error) {
	t.Helper()
	prevInfra, prevSignal, prevLiveness := killByPIDInfrastructureTarget, killByPIDSignal, killByPIDLiveness
	killByPIDInfrastructureTarget, killByPIDSignal = infra, kill
	killByPIDLiveness = func(int) (func(int) bool, func(int) bool) { return alive, alive }
	t.Cleanup(func() {
		killByPIDInfrastructureTarget, killByPIDSignal, killByPIDLiveness = prevInfra, prevSignal, prevLiveness
	})
}

// TestKillByPIDRefusesInfrastructureTarget fails immediately without the
...
func TestKillByPIDRefusesInfrastructureTarget(t *testing.T) {
	stubKillByPIDDeps(t,
		func(pid int) bool { return pid == 4242 },
		func(int) bool { return true },
		func(pid int, sig syscall.Signal) error {
			t.Fatalf("KillByPID signaled %d with %v: the infrastructure guard is not wired in", pid, sig)
			return nil
		},
	)
	if err := KillByPID(4242); err != nil {
		t.Fatalf("KillByPID(infrastructure) = %v, want nil: refusing is success", err)
	}
}
