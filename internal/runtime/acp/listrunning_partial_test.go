...
// an agent too loaded to service its control socket inside the probe budget —
// and it is deliberately distinct from staleControlSocket below.
func stuckControlSocket(t *testing.T, p *Provider, name string) {
	t.Helper()
	if err := runtime.WritePrivateFile(p.sockNamePath(name), []byte(name)); err != nil {
...
// since the monitor goroutine that unlinks the socket never got to run. The
// kernel refuses the connection, which is POSITIVE proof the session is gone.
...
func startLiveSession(t *testing.T, p *Provider) string {
	t.Helper()
	name := testName()
...
func TestListRunningPrefixFilterPrecedesProbe(t *testing.T) {
	...
	names, err := p.ListRunning(live)
	if err != nil {
		t.Fatalf("ListRunning(%q) = %v; an out-of-prefix stuck probe must not poison a narrow listing", live, err)
	}
	if !slices.Contains(names, live) {
		t.Fatalf("ListRunning(%q) = %v, want the running session listed", live, names)
	}
}
