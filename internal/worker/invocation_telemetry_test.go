...
// writeGeminiWorkerProject lays out a gemini project mapping the way
// gemini-cli does: searchBase/<dir>/.project_root containing the workdir,
// with chats/ holding the session recording.
func writeGeminiWorkerProject(t *testing.T, searchBase, workDir, sessionFile, content string) {
	t.Helper()
	projDir := filepath.Join(searchBase, "proj")
	...
	path := filepath.Join(chatsDir, sessionFile)
	if err := os.WriteFile(path, []byte(content), 0o644); err != nil {
		t.Fatalf("WriteFile(%q): %v", path, err)
	}
}

const geminiWorkerLegacySession = `{
  ...
  "messages": [
    ...
    {"id":"b25da806-1959-4f73-ae67-a512dead6002","timestamp":"2026-06-10T10:00:03.000Z","type":"gemini","content":"older answer","model":"gemini-3-pro","tokens":{"input":5000,"output":900,"cached":4000,"thoughts":100,"tool":0,"total":6000}},
    {"id":"c25da806-1959-4f73-ae67-a512dead6003","timestamp":"2026-06-10T10:00:05.000Z","type":"gemini","content":"answer","model":"gemini-3-pro","tokens":{"input":900,"output":60,"cached":800,"thoughts":20,"tool":0,"total":980}}
  ],
  "kind": "main"
}`

// assertGeminiNewestRecorded checks the no-cursor conservative rule against
// the newest gemini message of the fixtures above, then that a second prompt
// op records nothing new and the cursor landed on the message id.
func assertGeminiNewestRecorded(t *testing.T, reader *sdkmetric.ManualReader, handle *SessionHandle, store *beads.MemStore) {
	t.Helper()
	if _, err := handle.Message(context.Background(), MessageRequest{Text: "hello"}); err != nil {
		t.Fatalf("Message: %v", err)
	}
	...
	out := collectInvocationMetrics(t, reader)
	wantTokens := map[string]int64{
		"gc.agent.tokens.input":      900 - 800,
		"gc.agent.tokens.output":     60 + 20,
		"gc.agent.tokens.cache_read": 800,
	}
	for name, want := range wantTokens {
		got, attrSets := invocationInt64Total(out, name)
		if got != want {
		...
		if got := attrSets[0]["provider"]; got != "gemini" {
			t.Errorf("%s: provider = %q, want gemini", name, got)
		}
		if got := attrSets[0]["model"]; got != "gemini-3-pro" {
			t.Errorf("%s: model = %q, want gemini-3-pro", name, got)
		}
	}
	if _, costDPs := invocationCostTotal(out); costDPs != 0 {
		t.Errorf("gc.agent.invocation.cost_usd: %d datapoints, want 0 without configured gemini pricing", costDPs)
	}
	...
	if got := bead.Metadata[sessionpkg.MetadataKeyInvocationUsageCursor]; got != "c25da806-1959-4f73-ae67-a512dead6003" {
		t.Fatalf("cursor metadata = %q, want the newest gemini message id", got)
	}
...
func TestMessageRecordsGeminiInvocationTokens(t *testing.T) {
	reader := setupInvocationMetricsReader(t)
	handle, store, searchBase, workDir := newFamilyTelemetryHandle(t, ProfileGeminiTmuxCLI, "gemini", "gemini", nil)
	writeGeminiWorkerProject(t, searchBase, workDir, "session-2026-06-10T10-00-24e7203e.json", geminiWorkerLegacySession)
	assertGeminiNewestRecorded(t, reader, handle, store)
}

func TestMessageRecordsGeminiInvocationTokensJSONL(t *testing.T) {
	reader := setupInvocationMetricsReader(t)
	handle, store, searchBase, workDir := newFamilyTelemetryHandle(t, ProfileGeminiTmuxCLI, "gemini", "gemini", nil)

	// gemini-cli >=0.45 JSONL recording: metadata first line, the last
	// gemini message re-appended once its tokens arrive, $set patches
	// interleaved.
	jsonl := strings.Join([]string{
		...
		`{"id":"a25da806-1959-4f73-ae67-a512dead6001","timestamp":"2026-06-10T10:00:01.000Z","type":"user","content":"hello"}`,
		`{"id":"b25da806-1959-4f73-ae67-a512dead6002","timestamp":"2026-06-10T10:00:03.000Z","type":"gemini","content":"older answer","model":"gemini-3-pro","tokens":{"input":5000,"output":900,"cached":4000,"thoughts":100,"tool":0,"total":6000}}`,
		`{"id":"c25da806-1959-4f73-ae67-a512dead6003","timestamp":"2026-06-10T10:00:05.000Z","type":"gemini","content":"answer","model":"gemini-3-pro","tokens":null}`,
		`{"id":"c25da806-1959-4f73-ae67-a512dead6003","timestamp":"2026-06-10T10:00:05.000Z","type":"gemini","content":"answer","model":"gemini-3-pro","tokens":{"input":900,"output":60,"cached":800,"thoughts":20,"tool":0,"total":980}}`,
		`{"$set":{"lastUpdated":"2026-06-10T10:00:06.000Z"}}`,
	}, "\n") + "\n"
	writeGeminiWorkerProject(t, searchBase, workDir, "session-2026-06-10T10-00-24e7203e.jsonl", jsonl)
	assertGeminiNewestRecorded(t, reader, handle, store)
}
