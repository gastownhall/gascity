...
func TestPhase2CatalogDataIsAuthoritative(t *testing.T) {
	expected := []Requirement{
		...
		{Code: RequirementInputInProgressResumeRestart, Group: "input_delivery", Description: "A resumed session with in-progress assigned work receives a restart turn instead of landing idle when no explicit nudge is configured; hook-primed providers (opencode, mimocode) resume idle instead, their role supplied by the hook."},
		{Code: RequirementInputPreClaimResumeRestart, Group: "input_delivery", Description: "A resumed session with pre-claim work demand receives a restart turn instead of landing idle when no explicit nudge is configured; hook-primed providers (opencode, mimocode) resume idle instead, their role supplied by the hook."},
		{Code: RequirementInputHookPrimedResumeRoleOmitted, Group: "input_delivery", Description: "A resumed session whose installed provider hook supplies the rendered role prompt to every generation delivers the beacon plus the configured nudge as its restart turn and no restart turn when none is configured; every other profile keeps the rendered prompt in the restart turn."},
