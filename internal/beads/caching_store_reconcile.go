...
func (c *CachingStore) nextReconcileDelay(now time.Time) time.Duration {
	...
	// An outstanding failed-refresh publication must not be postponed
	// indefinitely: lastFullScanAt/lastFreshAt advance on every write, so a busy
	// cache would keep pushing the full-scan deadline forward and never recover
	// the intent. Anchor an independent deadline on the OLDEST pending event's
	// occurrence time so recovery becomes due within one adaptive interval of the
	// intent itself, then take the sooner of the two deadlines.
	//
	// A persistently unreadable intent would otherwise pin this at 0 forever
	// (its occurrence time never advances), spinning the full scan at the poll
	// floor. Once targeted recovery has failed for it, anchor the deadline on the
	// LAST recovery attempt and grow it with a bounded exponential backoff so the
	// scan cadence relaxes toward the failure backoff while the row stays broken.
	if c.pendingPublications.hasAny() {
		interval := c.adaptiveIntervalLocked()
		var pendingDueAt time.Time
		if c.pendingRecoveryFailures > 0 && !c.lastPendingRecoveryAt.IsZero() {
			pendingDueAt = c.lastPendingRecoveryAt.Add(pendingRecoveryBackoff(interval, c.pendingRecoveryFailures))
		} else if anchor := c.pendingPublications.oldestPendingOccurredAt(); !anchor.IsZero() {
			pendingDueAt = anchor.Add(interval)
		}
		if !pendingDueAt.IsZero() {
			pendingDelay := time.Duration(0)
			if now.Before(pendingDueAt) {
				pendingDelay = pendingDueAt.Sub(now)
			}
			if pendingDelay < delay {
				delay = pendingDelay
			}
		}
	}
...
// pendingRecoveryBackoff grows the pending reconcile deadline as
// interval * 2^min(failures, cap), clamped at cacheReconcileFailureBackoff. The
// first failure still keeps an eager (one-interval) deadline; only sustained
// failures relax the cadence.
func pendingRecoveryBackoff(interval time.Duration, failures int) time.Duration {
	if failures <= 0 {
		return interval
	}
	shift := failures - 1
	if shift > cachePendingRecoveryBackoffShiftCap {
		shift = cachePendingRecoveryBackoffShiftCap
	}
	backoff := interval << shift
	if backoff <= 0 || backoff > cacheReconcileFailureBackoff {
		return cacheReconcileFailureBackoff
	}
	return backoff
}

// recordPendingRecoveryOutcomeLocked updates the bounded backoff state after a
// reconcile pass attempted targeted pending-intent recovery. A pass that
// recovered at least one intent (progress) or hit no read fault resets the
// failure counter; a pass that only faulted with no progress advances it.
// Caller must hold c.mu.
func (c *CachingStore) recordPendingRecoveryOutcomeLocked(now time.Time, attempted, progressed, failed bool) {
	if !attempted {
		c.pendingRecoveryFailures = 0
		return
	}
	c.lastPendingRecoveryAt = now
	if failed && !progressed {
		if c.pendingRecoveryFailures < cachePendingRecoveryBackoffShiftCap+1 {
			c.pendingRecoveryFailures++
		}
		return
	}
	c.pendingRecoveryFailures = 0
...
func (c *CachingStore) runReconciliation() {
	...
	startGeneration := c.sharedMutationGeneration()
	c.mu.RUnlock()
	pendingSnapshot := c.pendingPublications.snapshot()
	...
	if err != nil {
		...
		c.updateStatsLocked()
		c.mu.Unlock()
		// A full scan is unavailable, but a failed-refresh publication whose
		// complete payload can still be read by a targeted Get/DepList must not be
		// stranded behind an unrelated List outage. Run the pending intents
		// through their own recovery pass before returning.
		c.recoverPendingPublicationsOnListFailure(pendingSnapshot)
		return
	}
	...
	pendingRecoveries, pendingNotFound, recoveryFailed := c.readPendingPublications(pendingSnapshot)

	// A reconciliation snapshot can discover IDs generated by an in-flight
	// Create before that Create has returned the ID and reserved its per-ID
	// mutation lock. Treat cache finalization as an unknown-scope mutation so it
	// serializes behind every backing commit already in flight. Sequence fences
	// still reject snapshots made stale by the mutation that won the scope.
	// Backing scans and observer callbacks remain outside the write scope.
	unlockMutation := c.lockUnknownMutationScope()
	pendingRecoveries = c.validatePendingRecoveries(pendingRecoveries)
	c.mu.Lock()
	now := time.Now()
	res := c.mergeSnapshotLocked(freshByID, confirmedClosed, depMap, useFreshDeps, startSeq, startGeneration, now)
	c.applyPendingRecoveriesLocked(&res, pendingRecoveries, freshByID, now)
	gateDrains := c.clearAbandonedPendingPublicationsLocked(pendingNotFound, freshByID)
	c.recordPendingRecoveryOutcomeLocked(now, len(pendingSnapshot) > 0, len(pendingRecoveries) > 0, recoveryFailed)
	...
	c.mu.Unlock()
	if c.reconcileAfterMergeBeforeNotificationReservationForTest != nil {
		c.reconcileAfterMergeBeforeNotificationReservationForTest()
	}
	drains := c.enqueueReconcileChanges(res.notifications)
	drains = append(drains, gateDrains...)
	unlockMutation()
	if emit {
		log.Print(logLine)
	}
...
// clearAbandonedPendingPublicationsLocked drops any pending intent whose bead
// was authoritatively deleted out-of-band: absent from the full-scan snapshot,
// absent from the cache, and read as ErrNotFound this pass. Its reserved gate
// placeholder would otherwise block that ID's ordered queue forever. Returns any
// drain funcs produced by releasing a blocked queue; the caller runs them after
// releasing its locks. Caller must hold c.mu.
func (c *CachingStore) clearAbandonedPendingPublicationsLocked(notFound map[string]cachePendingPublication, freshByID map[string]Bead) []func() {
	if len(notFound) == 0 {
		return nil
	}
	var drains []func()
	for id, publication := range notFound {
		if _, inFresh := freshByID[id]; inFresh {
			continue
		}
		if _, cached := c.beads[id]; cached {
			continue
		}
		if !c.pendingPublications.clearIfToken(id, publication.token) {
			continue
		}
		if drain := c.releaseOrderedPendingGate(id, publication.gate); drain != nil {
			drains = append(drains, drain)
		}
		c.recordProblemLocked(
			"recover pending observer publication",
			fmt.Errorf("%s: definite publication lost to out-of-band delete", id),
		)
	}
	return drains
}

// recoverPendingPublicationsOnListFailure publishes any failed-refresh
// publication whose complete payload can still be read by a targeted Get/DepList
// even though the reconcile full scan is unavailable. It mirrors the pending
// slice of runReconciliation's success path: read the targeted snapshots, fence
// them under the shared mutation scope, apply them onto an empty merge result,
// then drain after releasing the scope so observer callbacks may re-enter.
func (c *CachingStore) recoverPendingPublicationsOnListFailure(pendingSnapshot map[string]cachePendingPublication) {
	if len(pendingSnapshot) == 0 {
		return
	}
	pendingRecoveries, _, _ := c.readPendingPublications(pendingSnapshot)
	if len(pendingRecoveries) == 0 {
		return
	}
	unlockMutation := c.lockUnknownMutationScope()
	pendingRecoveries = c.validatePendingRecoveries(pendingRecoveries)
	now := time.Now()
	var res mergeSectionResult
	c.mu.Lock()
	c.applyPendingRecoveriesLocked(&res, pendingRecoveries, nil, now)
	c.mu.Unlock()
	drains := c.enqueueReconcileChanges(res.notifications)
	unlockMutation()
	for _, drain := range drains {
...
type mergeRowInput struct {
	...
	beadAtSeq     uint64
	startSeq      uint64
	sharedChanged bool // another cache handle may have changed this id after the scan began
	localAt       time.Time
...
func reconcileMergeDecision(in mergeRowInput) mergeDecision {
	...
	case in.freshExists: // absorb-loop cell
		if in.sharedChanged || in.deletedAtSeq > in.startSeq || in.beadAtSeq > in.startSeq {
...
func (c *CachingStore) mergeSnapshotLocked(
	...
	startSeq, startGeneration uint64, now time.Time,
) mergeSectionResult {
	...
	for id, freshBead := range freshByID {
		...
		d := reconcileMergeDecision(mergeRowInput{
			...
			startSeq:      startSeq,
			sharedChanged: c.sharedMutationChangedSince(id, startGeneration),
	...
	for id, cached := range c.beads {
		...
		res.removes++
		if d.notification == "bead.closed" && !c.closeObserverSuppressedLocked(id) {
...
// reconcileNotificationPayload builds the complete observer payload for a
// reconciled add/update. A dependency-stripping backing (the fork's native
// DoltLite read store) returns List rows with no dependency fields; the
// authoritative deps arrive separately in depMap and are resolved into
// freshDeps. On that useFreshDeps path the published payload attaches them
// explicitly so observers and event export never see an incomplete reconciled
// snapshot. The cache row absorb keeps its List-compatible shape and is left
// untouched — only the observer payload is completed here.
func reconcileNotificationPayload(freshBead Bead, freshDeps []Dep, useFreshDeps bool) Bead {
	payload := cloneBead(freshBead)
	if useFreshDeps && len(freshDeps) > 0 && !beadCarriesDependencyFields(freshBead) {
		payload.Dependencies = cloneDeps(freshDeps)
		payload.Needs = nil
	}
	return payload
}
