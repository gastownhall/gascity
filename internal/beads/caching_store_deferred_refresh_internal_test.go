...
// TestLiveListRefreshConvergesOnNativelyBlockedBead mirrors
// TestLiveListRefreshConvergesOnIndefinitelyDeferredBead exactly, substituting
// bd's "blocked" status for "deferred". Same fixed point, same fix: bd's
// blocked status normalizes to Gas City's Status "open" (mapBdStatus's
// default case) with the fact preserved out-of-band in Bead.NativelyBlocked
// (bdstore.go normalizedBdReadState), so ListQuery.Matches keeps matching a
// Status "open" query for the cached copy while `bd list --status=open`
// keeps omitting it — stale again on every live list, forever, unless
// staleLiveCacheIDs is taught to skip it the same way it already skips an
// indefinitely-deferred bead.
func TestLiveListRefreshConvergesOnNativelyBlockedBead(t *testing.T) {
	const beadID = "ga-blocked1"
	...
	issue := func(status string) []byte {
		...
		// bdIssue instead serializes optionalBool is_blocked as {}, which the
		// decoder rejects as corrupt.
		return []byte(`[{"id":"` + beadID + `","title":"blockable work","status":"` + status +
			`","issue_type":"task","assignee":"gascity/builder"}]`)
	}
	...
	runner := func(_, name string, args ...string) ([]byte, error) {
		...
		case "list":
			...
			// bd filters on its OWN status vocabulary, before Gas City's
			// normalization: a blocked row is not an open row, so
			// --status=open stops returning it. An UNFILTERED bd list still
			// returns it, carrying bd's real status — which is why the
			// unfiltered reconcile scan can still see it.
			if hasArgPrefix(args, "--status=open") {
	...
	// Someone blocks it. bd list --status=open no longer returns it.
	mu.Lock()
	blockedNow = true
	showCalls = 0
	mu.Unlock()
	...
	// First live list: discovering the block costs one Get. That is the
	// refresh doing its job.
	if _, err := cache.List(liveQuery); err != nil {
		t.Fatalf("first live List: %v", err)
	}
	...
	mu.Unlock()
	if afterFirst == 0 {
		t.Fatalf("first live List issued no bd show; the refresh never noticed the block")
	}
	...
	if extra := total - afterFirst; extra != 0 {
		t.Fatalf("live-list refresh never converges: 3 further identical live lists issued %d more bd show calls for %s (cached Status=%q NativelyBlocked=%v). "+
			"Every natively-blocked bead in the cache is re-Got on every live list.",
			extra, beadID, cached.Status, cached.NativelyBlocked)
	}
}
