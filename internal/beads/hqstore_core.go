...
func (s *HQStore) candidateIDsLocked(q ListQuery) hqIDSet {
	switch q.TierMode {
	case TierWisps:
		// Wisps tier includes ephemeral beads (stored in s.wisps) AND no-history
		// beads (stored in s.main). Include the latter by scanning main for NoHistory.
		noHistoryIDs := make(hqIDSet)
		for id, b := range s.main {
			if b.NoHistory {
				noHistoryIDs[id] = struct{}{}
			}
		}
		return unionHQIDSets(s.wispIdx.candidateIDs(q), noHistoryIDs)
	case TierBoth:
