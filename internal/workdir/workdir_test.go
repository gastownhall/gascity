...
func TestResolveWorkDirPathUsesConfiguredWorktreesRoot(t *testing.T) {
	t.Setenv("T3CODE_WORKTREES_DIR", "/tmp/t3-worktrees")
	cityPath := t.TempDir()
	cfg := &config.City{
	...
	agent := config.Agent{
		Name:    "refinery",
		Dir:     "demo",
		WorkDir: "{{.WorktreesRoot}}/gascity/{{.Rig}}/{{.AgentBase}}",
	}

	got := ResolveWorkDirPath(cityPath, CityName(cityPath, cfg), "demo/refinery", agent, cfg.Rigs)
	want := filepath.Join("/tmp/t3-worktrees", "gascity", "demo", "refinery")
	if got != want {
		t.Fatalf("ResolveWorkDirPath() = %q, want %q", got, want)
	}
}
