...
for DB in $DATABASES; do
    ...
    REAP_COUNT=$(dolt_sql -r csv -q "
        ...
        LEFT JOIN \`$DB\`.wisp_dependencies d
            ON d.issue_id = w.id AND d.type = 'parent-child'
        LEFT JOIN \`$DB\`.issues parent_issue
            ON d.depends_on_id = parent_issue.id
        LEFT JOIN \`$DB\`.wisps parent_wisp
            ON d.depends_on_id = parent_wisp.id
        WHERE w.status IN ('open', 'hooked', 'in_progress')
        AND w.created_at < DATE_SUB(NOW(), INTERVAL $MAX_AGE_H HOUR)
        AND (
            d.depends_on_id IS NULL
            OR (parent_issue.id IS NULL AND parent_wisp.id IS NULL)
            OR parent_issue.status = 'closed'
            OR parent_wisp.status = 'closed'
        )
