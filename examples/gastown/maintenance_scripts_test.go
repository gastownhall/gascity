...
// wispTimestampLayout is a Go time-format layout that produces strings both
// GNU `date -d` and BSD `date -j -f "%Y-%m-%dT%H:%M:%S"` accept. Dropping
// the trailing `Z` from time.RFC3339 is what makes the BSD branch parse;
// pinning TZ=UTC in wispCompactEnv keeps the no-timezone string interpreted
// uniformly across Linux CI and macOS dev.
