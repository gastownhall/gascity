  ...
  if command -v "$GC_BIN" >/dev/null 2>&1; then
    ...
    if rig_json=$(run_bounded 5 "$GC_BIN" rig list --json 2>/dev/null); then
      ...
      if [ "$rig_status" -eq 124 ]; then
        printf 'gc-nudge: ${GC_BIN:-gc} rig list timed out after 5s; falling back to local filesystem metadata scan\n' >&2
      else
        printf 'gc-nudge: ${GC_BIN:-gc} rig list failed rc=%s; falling back to local filesystem metadata scan\n' "$rig_status" >&2
      fi
    fi
  fi
