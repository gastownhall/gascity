...
load_health_check_from_gc() {
    ...
    gc_bin=$(resolve_gc_helper_bin)
    [ -n "$gc_bin" ] || return 1
    # Routine health checks stay read-only. Use explicit read-only diagnostics
    # only in recovery paths to avoid write-probe churn against managed Dolt.
    output=$("$gc_bin" dolt-state health-check --host "$host" --port "$DOLT_PORT" --user "$DOLT_USER" </dev/null 2>/dev/null) || return 1
...
op_start() {
    ...
    if load_existing_managed_from_gc; then
        existing_pid="$GC_EXISTING_MANAGED_PID"
        if [ "$GC_EXISTING_REUSABLE" = "true" ] && [ -n "$GC_EXISTING_STATE_PORT" ] && { [ -z "$GC_DOLT_PORT" ] || [ "$GC_EXISTING_STATE_PORT" = "$GC_DOLT_PORT" ]; }; then
            DOLT_PORT="$GC_EXISTING_STATE_PORT"
            echo "$existing_pid" > "$PID_FILE"
            save_state "$existing_pid" true
            sync_port_files
