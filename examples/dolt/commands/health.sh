...
# Use pgrep -x to match only processes named "dolt", then verify each is
# actually running sql-server via ps. This avoids false positives from
# processes that merely mention "dolt" in their args (e.g., Claude sessions
# whose prompt text contains "dolt sql-server").
zombie_count=0
zombie_pids=""
